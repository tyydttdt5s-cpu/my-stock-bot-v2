...
if "股票代號" in stock_df.columns: 
    stock_code_col = "股票代號" 
elif "代碼" in stock_df.columns: 
    stock_code_col = "代碼" 
 
if stock_name_col is None: 
    raise Exception("找不到『股票名稱』欄位") 
 
if stock_code_col is None: 
    raise Exception("找不到『股票代號』或『代碼』欄位") 
 
stocks = dict( 
    zip( 
        stock_df[stock_name_col], 
        stock_df[stock_code_col] 
    ) 
) 
...
for row in range(2, ws.max_row + 1): 
    ...
    if score >= 80: 
        ws.cell(row, score_col).fill = green_fill 
    elif score >= 60: 
        ws.cell(row, score_col).fill = yellow_fill 
    else: 
        ws.cell(row, score_col).fill = red_fill 
    ...
    if token and user_id:
        ...
        if not hold_df.empty:
            ...
            for _, row in hold_df.iterrows():
                ...
                change_symbol = "📈" if row["今日漲跌%"] > 0 else "📉"

                message += (
                    f"{row['股票代號']} {row['股票']}\n"
                    f"評分：{row['技術評分']}\n"
                    f"漲跌：{change_symbol} {row['今日漲跌%']}%\n"
                    f"建議：{row['AI建議']}\n\n"
                )
        ...
        print(f"LINE推播完成 ({response.status_code})")

    else:

        print("找不到 LINE_CHANNEL_ACCESS_TOKEN 或 LINE_USER_ID")
